...
const Sidebar: React.FC = () => {
  ...
  return (
    <div
      className={`bg-gray-200 text-white h-screen fixed z-10 ${isCollapsed ? 'w-16' : 'w-64'} transition-all duration-300 flex flex-col md:static `}
    >
          ...
          <div key={category.category} className="">
            ...
            {category.items.map((item) => (
              <Link
                key={item.name}
                to={item.path}
                className={`flex items-center ${isCollapsed ? 'justify-center' : 'justify-start'} gap-4 px-4 py-2 hover:bg-gray-600 dark:hover:bg-gray-700 text-black dark:text-gray-300 rounded-lg `}
              >
